...
void wellPopup::Init(CDeviceCursor* cursor, CMaterial* mat1, CWritingFont* font) {
	...
	m_headlineW.PrintString("Brunnen");
	...
	dummystring = "Wasser pro Stunde: " + std::to_string(10);
	...
	m_infotext1.PrintString("Wasser pro Stunde bestimmt wie viel Wasser her-");

	m_infotext2.Init(CFloatRect(0.1, 0.75, 0.7, 0.2), 50, font);
	m_infotext2.SetLayer(0.97);
	m_infotext2.SetInnerOn();
	m_infotext2.PrintString("gestellt wird, manche Gebaeude brauchen Wasser.");
...
void wellPopup::setInfo(int powerUse, int waterUse, int NutrientUse, int howMuch)
{
	std::string dumyString;
	dumyString = "Wasser pro Stunde: " + std::to_string(howMuch);
	m_info1W.PrintString(&dumyString[0]);

	dumyString = "Stromverbrauch: " + std::to_string(powerUse);
	m_info2W.PrintString(&dumyString[0]);

	dumyString = "Wasserverbrauch: " + std::to_string(0);
